export type StarterTaskCategory =
  | 'watering' | 'fertilizing' | 'pruning' | 'planting'
  | 'harvesting' | 'pest_control' | 'composting' | 'general';

export type StarterTask = {
  title: string;                 // Hebrew, short
  notes: string;                 // Hebrew, 1-2 sentences
  category: StarterTaskCategory;
  dayOffset: number;             // due date = today + dayOffset (days)
...
export function getStarterTasks(plantType?: string | null): StarterTask[] {
  return STARTER_TASKS[plantType ?? ''] ?? STARTER_TASKS.default;
}
