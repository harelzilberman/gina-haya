    ...
    return (
      <div style={{ textAlign: 'center', padding: '60px 0' }}>
        ...
        <p style={{ fontFamily: DM_SANS, fontSize: '13px', color: MUTED }}>
          {isHe ? 'היה ראשון לשתף!' : 'Be the first to share!'}
        </p>
      </div>
    );
  }

  // Masonry-style grid: entries with photos get a bigger card
  return (
    <div dir={isRTL ? 'rtl' : 'ltr'}>
      <div style={{
        columns: '2 280px', columnGap: '12px',
      }}>
        {entries.map(entry => (
          ...
          >
            {/* Photo(s) if any — just show first as hero */}
              ...
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '6px' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '7px' }}>
                  <div style={{
                    ...
                    display: 'flex', alignItems: 'center', justifyContent: 'center',
                    fontFamily: FRANK, fontSize: '12px', fontWeight: 700, color: BIO_CYAN,
                  }}>
                    {(entry.users?.display_name || '?')[0].toUpperCase()}
                  </div>
                  <span style={{ fontFamily: DM_SANS, fontSize: '12px', color: TEXT_MID }}>
                    {entry.users?.display_name || (isHe ? 'גנן/ת' : 'Gardener')}
                  </span>
                </div>
